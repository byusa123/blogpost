...
class CommentModelTest(unittest.TestCase):
    def setUp(self):
        self.user_byusa = User(username='gizo', password='12345', email='test@test.com')
        self.new_blog = Blog(id=1, title='Test', content='This is a test blog', user_id=self.user_byusa.id)
        self.new_comment = Comment(id=1, comment ='This is a test comment', user_id=self.user_byusa.id, blog_id = self.new_blog.id )
    ...
    def test_check_instance_variables(self):
        ...
        self.assertEquals(self.new_comment.user_id, self.user_byusa.id)
